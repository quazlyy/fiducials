    ...
    for (int indCornerGroup = 0; indCornerGroup < cornerGroups.size(); indCornerGroup++) {
        ...
        for (unsigned int cornerInd = 0; cornerInd < currCornerGroup.size(); cornerInd++) {
            int i1 = cornerInd, i2 = (i1 + 1) % currCornerGroup.size(),
                i3 = (i1 + 2) % currCornerGroup.size(), i4 = (i1 + 3) % currCornerGroup.size();

            Corner c1 = currCornerGroup[i1], c2 = currCornerGroup[i2], c3 = currCornerGroup[i3],
                   c4 = currCornerGroup[i4];

            // if there are only 3 corners, replace the 4th corner with an invalid one
            if (i1 == i4) c4 = Corner(Point2d(INFINITY, INFINITY), LineSegment(), LineSegment());

            vector<Corner> corners = {c1, c2, c3, c4};

            if (!checkIfCornersFormQuad(corners, edInterface)) continue;

            vector<Point2d> cornerLocs = {corners[0].loc, corners[1].loc, corners[2].loc,
                                          corners[3].loc};
...
const vector<vector<Corner>>& QuadDetector::getCornerGroups() { return cornerGroups; }

const vector<Quad>& QuadDetector::getQuads() const { return quads; }

const vector<Quad>& QuadDetector::getDistortedQuads() const { return distortedQuads; }
...
void QuadDetector::detectCorners(EDInterface* edInterface, const vector<vector<int>>& lineGroups) {
    ...
    for (int lineGroupInd = 0; lineGroupInd < lineGroups.size(); lineGroupInd++) {
        ...
        for (int lineInd = 0; lineInd < lineGroups[lineGroupInd].size(); lineInd++) {
            ...
            for (int edgePixInd = 0; edgePixInd < edgeMap->segments[line1.segmentNo].noPixels;
                 edgePixInd++) {
                if (abs(edgeMap->segments[line1.segmentNo].pixels[edgePixInd].c - inters.x) +
                        abs(edgeMap->segments[line1.segmentNo].pixels[edgePixInd].r - inters.y) <
                    thresManhDist) {
                    onTheSegment = true;
                    break;
                }
...
bool QuadDetector::checkIfCornersFormQuad(vector<Corner>& corners, EDInterface* edInterface) {
    ...
    estC1 = Corner(edInterface->intersectionOfLineSegments(corners[0].l1, corners[2].l1),
                   corners[0].l1, corners[2].l1);
    estC3 = Corner(edInterface->intersectionOfLineSegments(corners[0].l2, corners[2].l2),
                   corners[0].l2, corners[2].l2);
    vector<Corner> estCorners = {corners[0], estC1, corners[2], estC3};
    if (!checkIfQuadIsSimple(estCorners)) {
        estC1 = Corner(edInterface->intersectionOfLineSegments(corners[0].l1, corners[2].l2),
                       corners[0].l1, corners[2].l2);
        estC3 = Corner(edInterface->intersectionOfLineSegments(corners[0].l2, corners[2].l1),
    ...
    if ((distC1estC1 < distC1estC3) && (distC1estC1 < distC3estC1) && (distC1estC1 < distC3estC3) &&
        (distC1estC1 < thresDistSquared)) {
...
bool QuadDetector::checkIfQuadIsSimple(const vector<Corner>& corners) {
    ...
    if (crossProduct(vec13, vec12) * crossProduct(vec13, vec14) >= 0) return false;
    ...
    if (crossProduct(vec24, vec21) * crossProduct(vec24, vec23) >= 0) return false;
