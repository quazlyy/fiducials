...
cv::Mat Stag::drawMarkers() { return drawer.drawMarkers("markers.png", image, markers); }
...
void Stag::checkDuplicate(Marker mrkr) {
    if (markers.size() == 0)
        ...
        if (notFound) markers.emplace_back(mrkr);
    }
}
...
Codeword Stag::readCode(const Quad &q) {
    ...
    for (int i = 0; i < 48; i++) {
        ...
        samples[i] = readPixelSafeBilinear(
            image, Point2d(projectedPoint.at<double>(0) / projectedPoint.at<double>(2),
...
void Stag::fillCodeLocations() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        codeLocs[2 + i * 12] = createMatFromPolarCoords(
            0.206935928182607, HALF_PI - 0.459275804122858 + i * HALF_PI, innerCircleRadius);
        ...
        codeLocs[5 + i * 12] = createMatFromPolarCoords(
            0.327493143484516, HALF_PI - 0.591687617505840 + i * HALF_PI, innerCircleRadius);
        codeLocs[6 + i * 12] = createMatFromPolarCoords(
            0.313672146827381, HALF_PI - 0.200579720495241 + i * HALF_PI, innerCircleRadius);
        ...
        codeLocs[10 + i * 12] = createMatFromPolarCoords(
            0.437226762361658, HALF_PI - 0.433363129825345 + i * HALF_PI, innerCircleRadius);
        codeLocs[11 + i * 12] = createMatFromPolarCoords(
            0.437421957035861, HALF_PI - 0.145724938287167 + i * HALF_PI, innerCircleRadius);
    }
