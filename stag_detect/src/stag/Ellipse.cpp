...
void AperB(double **_A, double **_B, double **_res, int _righA, int _colA, int _righB, int _colB) {
    ...
    for (p = 1; p <= _righA; p++)
        for (q = 1; q <= _colB; q++) {
            _res[p][q] = 0.0;
            for (l = 1; l <= _colA; l++) _res[p][q] = _res[p][q] + _A[p][l] * _B[l][q];
        }
}

void A_TperB(double **_A, double **_B, double **_res, int _righA, int _colA, int _righB,
             int _colB) {
    int p, q, l;
    for (p = 1; p <= _colA; p++)
        for (q = 1; q <= _colB; q++) {
            _res[p][q] = 0.0;
            for (l = 1; l <= _righA; l++) _res[p][q] = _res[p][q] + _A[l][p] * _B[l][q];
        }
}

void AperB_T(double **_A, double **_B, double **_res, int _righA, int _colA, int _righB,
             int _colB) {
    int p, q, l;
    for (p = 1; p <= _colA; p++)
        for (q = 1; q <= _colB; q++) {
            _res[p][q] = 0.0;
            for (l = 1; l <= _righA; l++) _res[p][q] = _res[p][q] + _A[p][l] * _B[q][l];
        }
}
...
customEllipse::customEllipse(pix *points, int noPnts) {
    ...
    } else if (B1 != 0)  // Rotate the axes
    {
        ...
        A2 = 0.5 *
             (A1 * (1 + cos(2 * rotation) + B1 * sin(2 * rotation) + C1 * (1 - cos(2 * rotation))));
...
double customEllipse::GetSquaredDistance(double pX, double pY, double &estimation) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        distances[i] =
            SquaredDistanceBwPoints(transPntX, transPntY, candidatesX[i], candidatesY[i]);
...
///
bool CircleFit(const vector<double> &Xs, const vector<double> &Ys, double &centerX, double &centerY,
               double &radius) {
    int N = Xs.size();
