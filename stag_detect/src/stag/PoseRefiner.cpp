...
void PoseRefiner::refineMarkerPose(EDInterface* edInterface, Marker& marker) {
    ...
    static const double sinVals[36] = {
        0.000000,  0.173648,  0.342020,  0.500000,  0.642788,  0.766044,  0.866025,  0.939693,
        0.984808,  1.000000,  0.984808,  0.939693,  0.866025,  0.766044,  0.642788,  0.500000,
        0.342020,  0.173648,  0.000000,  -0.173648, -0.342020, -0.500000, -0.642788, -0.766044,
        -0.866025, -0.939693, -0.984808, -1.000000, -0.984808, -0.939693, -0.866025, -0.766044,
        -0.642788, -0.500000, -0.342020, -0.173648};
    ...
    for (int indEdgeSegment = 0; indEdgeSegment < edgeMap->noSegments; indEdgeSegment++) {
        ...
        if (edgeMap->segments[indEdgeSegment].noPixels < minEdgeSegmentLength) continue;

        // skip if the edge segment is not a loop
        if (squaredDistance(Point2d(edgeMap->segments[indEdgeSegment].pixels[0].c,
        ...
        for (int indEdgePix = 0; indEdgePix < edgeMap->segments[indEdgeSegment].noPixels;
             indEdgePix += minEdgeSegmentLength) {
            if (!checkIfPointInQuad(
                    marker, Point2d(edgeMap->segments[indEdgeSegment].pixels[indEdgePix].c,
        ...
        vector<double> pixelErrors(edgeMap->segments[indEdgeSegment].noPixels,
                                   INFINITY);  // min distance from edge segment individual edge
                                               // segment pixels to the 36 points
        for (int indEdgePix = 0; indEdgePix < edgeMap->segments[indEdgeSegment].noPixels;
             indEdgePix++) {
            Mat edgePix =
                (cv::Mat_<double>(3, 1) << edgeMap->segments[indEdgeSegment].pixels[indEdgePix].c,
                 edgeMap->segments[indEdgeSegment].pixels[indEdgePix].r, 1);
            Mat projEdgePix = Hinv * edgePix;

            projPixels[indEdgePix] = Point2d(projEdgePix.at<double>(0) / projEdgePix.at<double>(2),
                                             projEdgePix.at<double>(1) / projEdgePix.at<double>(2));
            for (int sampleInd = 0; sampleInd < 36; sampleInd++) {
                double dist = sqrt((projPixels[indEdgePix].x - samplePoints[sampleInd].x) *
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) x.at<double>(i + j * 3) = marker.H.at<double>(i, j);
    ...
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++) marker.H.at<double>(i, j) = x.at<double>(i + j * 3);
