...
/// to the same segment
void JoinCollinearLines(EDLines *lines, double MAX_DISTANCE_BETWEEN_TWO_LINES = 6.0,
    ...
    while (noPixels >= MIN_LINE_LEN) {
        ...
        while (index < noPixels) {
            ...
            if (goodPixelCount < 2 || index >= noPixels) {
                ...
                int index = 0;
                while (ComputeMinDistance(x[index], y[index], lastA, lastB, lastInvert) >
                       LINE_ERROR)
                    index++;
...
void ValidateLineSegments(EdgeMap *map, unsigned char *srcImg, EDLines *lines,
    ...
    bool ValidateLineSegmentRect(unsigned char *srcImg, int width, int height, int *x, int *y,
                                 LineSegment *ls, NFALUT *LUT);
    // Timer timer;
...
///
void EnumerateRectPoints(double sx, double sy, double ex, double ey, int ptsx[], int ptsy[],
                         int *pNoPoints) {
...
bool ValidateLineSegmentRect(unsigned char *srcImg, int width, int height, int *x, int *y,
                             LineSegment *ls, NFALUT *LUT) {
    void EnumerateRectPoints(double sx, double sy, double ex, double ey, int x[], int y[],
                             int *pNoPoints);
...
//   for (int i=0; i<width*height; i++) if (gradImg[i] >= GRADIENT_THRESH) dirImg[i]=255; else
//   dirImg[i]=0; SaveImage("OutputImages/gradImgBW.pgm", (char *)dirImg, width, height, 8);
// #endif
...
EDLines *DetectLinesByEDPF(EdgeMap *&map, unsigned char *srcImg, int width, int height,
                           bool flagOnlyUseEdgeSegmentLoops, int thresManhDist) {
