...
void Drawer::drawEdgeMap(const string& path, Mat image, EdgeMap* edgeMap) {
    ...
    for (int i = 0; i < edgeMap->noSegments; i++) {
        for (int j = 0; j < edgeMap->segments[i].noPixels; j++) {
            colorAPixel(bgrMat, edgeMap->segments[i].pixels[j].c, edgeMap->segments[i].pixels[j].r,
                        cv::Scalar(255, 255, 255), whiteDotWidth);
        }
    }
...
void Drawer::drawLines(const string& path, Mat image, EDLines* edLines) {
    ...
    for (int i = 0; i < edLines->noLines; i++) {
        if (edLines->lines[i].segmentNo != currSegment) currSegment = edLines->lines[i].segmentNo;

        cv::line(bgrMat, cv::Point(edLines->lines[i].sx, edLines->lines[i].sy),
                 cv::Point(edLines->lines[i].ex, edLines->lines[i].ey), cv::Scalar(255, 255, 255),
                 3, cv::LINE_AA);
        cv::line(bgrMat, cv::Point(edLines->lines[i].sx, edLines->lines[i].sy),
                 cv::Point(edLines->lines[i].ex, edLines->lines[i].ey), colors[i % colors.size()],
                 2, cv::LINE_AA);
    }
    // vector<int> compressionParams = { CV_IMWRITE_PNG_COMPRESSION, 0 };
    vector<int> compressionParams = {cv::IMWRITE_PNG_COMPRESSION, 0};
...
void Drawer::drawCorners(const string& path, Mat image,
    ...
    for (int i = 0; i < cornerGroups.size(); i++) {
        for (int j = 0; j < cornerGroups[i].size(); j++) {
            cv::circle(bgrMat, cv::Point(cornerGroups[i][j].loc.x, cornerGroups[i][j].loc.y), 4,
                       cv::Scalar(255, 255, 255), -1, cv::LINE_AA);
            cv::circle(bgrMat, cv::Point(cornerGroups[i][j].loc.x, cornerGroups[i][j].loc.y), 3,
...
cv::Mat Drawer::drawMarkers(const string& path, Mat image, const vector<Marker>& markers) {
    ...
    for (int i = 0; i < markers.size(); i++) {
        ...
        cv::circle(bgrMat, cv::Point(corners[0].x, corners[0].y), 6, cv::Scalar(255, 255, 255), -1,
                   cv::LINE_AA);
        for (int j = 0; j < 4; j++)
            cv::line(bgrMat, cv::Point(corners[j].x, corners[j].y),
                     cv::Point(corners[(j + 1) % 4].x, corners[(j + 1) % 4].y),
        ...
        cv::circle(bgrMat, cv::Point(corners[0].x, corners[0].y), 5, cv::Scalar(250, 50, 50), -1,
                   cv::LINE_AA);
        cv::circle(bgrMat, cv::Point(corners[1].x, corners[1].y), 5, cv::Scalar(50, 250, 50), -1,
                   cv::LINE_AA);
        cv::circle(bgrMat, cv::Point(corners[2].x, corners[2].y), 5, cv::Scalar(50, 50, 250), -1,
                   cv::LINE_AA);
        for (int j = 0; j < 4; j++)
            cv::line(bgrMat, cv::Point(corners[j].x, corners[j].y),
                     cv::Point(corners[(j + 1) % 4].x, corners[(j + 1) % 4].y),
        ...
        cv::circle(bgrMat, cv::Point(center.x, center.y), 6, cv::Scalar(255, 255, 255), -1,
                   cv::LINE_AA);
        cv::circle(bgrMat, cv::Point(center.x, center.y), 5, cv::Scalar(50, 255, 50), -1,
                   cv::LINE_AA);

        cv::putText(bgrMat, std::to_string(markers[i].id), center, cv::FONT_HERSHEY_DUPLEX, 2,
                    cv::Scalar(255, 255, 255), 5, cv::LINE_AA);
        cv::putText(bgrMat, std::to_string(markers[i].id), center, cv::FONT_HERSHEY_DUPLEX, 2,
                    cv::Scalar(50, 50, 255), 2, cv::LINE_AA);
    }
    return bgrMat;
    // vector<int> compressionParams = { CV_IMWRITE_PNG_COMPRESSION, 0 };
    // vector<int> compressionParams = {cv::IMWRITE_PNG_COMPRESSION, 0 };
    ...
    for (int i = 0; i < markers.size(); i++) {
        ...
        for (int y = 0; y < image.size().height; y++) {
            ...
            double c = markers[i].C.at<double>(1, 1) * y * y +
                       markers[i].C.at<double>(1, 2) * 2 * y + markers[i].C.at<double>(2, 2);
        ...
        for (int x = 0; x < image.size().width; x++) {
            ...
            double c = markers[i].C.at<double>(0, 0) * x * x +
                       markers[i].C.at<double>(0, 2) * 2 * x + markers[i].C.at<double>(2, 2);
